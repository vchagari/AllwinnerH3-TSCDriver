 ...
 * (C) 2013 thomas.schorpp@gmail.com
 *
...
static void release_tsc_pio(void)
{
	if (tsc_devp->pinctrl != NULL) {
	         printk(KERN_ALERT "inside release_tsc_pio:ifcondition\n");
	         devm_pinctrl_put(tsc_devp->pinctrl);
	         printk(KERN_ALERT "after the if loop\n");
     }
...
static int __init tscdev_init(void)
{
	...
	type = script_get_item("ts0", "tsc_used", &sval);
